...
void Intake::DoMoveDown() {
    m_tilter.SetControl(controls::PositionVoltage{units::turn_t{m_downPosition}});
}
...
void Intake::DoStow() {
    m_tilter.SetControl(controls::PositionVoltage{0_tr}.WithSlot(0));
}

void Intake::DoAngleForBump() {
    m_tilter.SetControl(controls::PositionVoltage{units::turn_t{14}}.WithSlot(0));
}
...
frc2::CommandPtr Intake::SlowPush() {
    return RunOnce([this] {
        DoIntake();
        m_tilter.SetControl(controls::PositionVoltage{units::turn_t{1.9}}.WithSlot(1));
    });
}
...
frc2::CommandPtr Intake::AutoZero() {
    return this->RunEnd(
        ...
        return m_tilter.GetStatorCurrent().GetValue().value() > 30.0;
    });
}
